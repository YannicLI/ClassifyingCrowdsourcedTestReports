...
def classifier(load_fn, appName, appID, row, col, xxx):
    ini_col = col
    ...
    # Image:
    i_train_data, i_train_target = i_cla.getVector(load_fn, appName, appID)
    ...
    for num in range(0, FREQUENCY):
        # print test_size
        train_data, test_data, train_target, test_target = train_test_split(all_train_data, np.array(all_train_target), test_size=1-xxx/10.0, random_state=0)
        ...
        ws.write(row, col, str(accuracy1))
        col += 1
        # # print "Precision:"
        ...
        ws.write(row, col, str(precision1))
        col += 1
        # # print "Recall:"
    ...
    ws.write(FREQUENCY + 2, col, str(accuracy / FREQUENCY))
...
if __name__ == "__main__":
    ...
    for x in xrange(9,10):
        rb = open_workbook('/Users/liyuying/Desktop/result'+str(x)+'.xls')
        wb = copy(rb)
        ws = wb.add_sheet("CNN")
        ...
        for i in range(0, 6):
            row = 2
            col = 4 * i + 1
            load_fn = 'Image/mfiles/result/' + appName[i] + '.mat'
            print appName[i]
            classifier(load_fn, appName[i], appID[i], row, col, x)

        wb.save('/Users/liyuying/Desktop/result'+str(x)+'.xls')
